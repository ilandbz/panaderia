...
const cerrarModal = () => {
  ...
  modalInstance.value = new bootstrap.Modal(document.getElementById('clienteQuickModal'));
...
<template>
  <div class="modal fade" id="clienteQuickModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content border-0 shadow-lg rounded-4">
        <div class="modal-header bg-primary text-white border-0 py-3">
          <h5 class="modal-title fw-bold"><i class="fas fa-user-plus me-2"></i> Nuevo Cliente</h5>
          ...
          <form @submit.prevent="save">
            <div class="row g-3">
              <div class="col-md-5">
                ...
                <select v-model="form.tipo_documento" class="form-select border-light bg-light" @change="form.razon_social = (form.tipo_documento === 'RUC' ? form.razon_social : '')">
              ...
              <div class="col-md-7">
                ...
                <input v-model="form.numero_documento" type="text" class="form-control border-light bg-light" placeholder="00000000" required>
              </div>
              <div class="col-12">
                <label class="form-label small fw-bold text-muted">Nombre Completo / Contacto</label>
                <input v-model="form.nombre_completo" type="text" class="form-control border-light bg-light" placeholder="Nombres y Apellidos" required>
              </div>
              <div v-if="form.tipo_documento === 'RUC'" class="col-12 animate__animated animate__fadeIn">
                <label class="form-label small fw-bold text-muted">Razón Social</label>
                <input v-model="form.razon_social" type="text" class="form-control border-light bg-light" placeholder="Nombre legal de la empresa" required>
              </div>
              <div class="col-12">
                <label class="form-label small fw-bold text-muted">Dirección (Opcional)</label>
                <input v-model="form.direccion" type="text" class="form-control border-light bg-light" placeholder="Av. Siempre Viva 123">
              </div>
            </div>
